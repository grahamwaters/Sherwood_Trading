    ...
    for crypto in tqdm(cryptos):
        try:
            ...
            try:
                ...
                logging.error(f'Error getting current_equity for {crypto}: {e}, line {traceback.tb_lineno()}')
                current_equity = profile_data['equity_previous_close']
            current_equity = float(current_equity)
            try:
                previous_equity = profile_data['adjusted_equity_previous_close']
            except Exception as e:
                logging.error(f'Error getting previous_equity for {crypto}: {e}, line {traceback.tb_lineno()}')
                previous_equity = profile_data['equity_previous_close']
