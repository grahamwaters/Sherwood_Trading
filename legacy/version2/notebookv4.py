        ...
        for index, row in tqdm(crypto_data.iterrows()):
            ...
            try:
                ...
                try:
                    ...
                    logging.warning(f'KeyError: {row["ticker"]}, set the daily_profit and min_order_size to 0.0')
                    daily_profit = 0.0
                    min_order_size = 0.0
                logging.warning(f'KeyError: {row["ticker"]}, set the date to 2021-01-01')
                updated_date = pd.to_datetime('2021-01-01')
            ...
            # if the total daily return is less than -2%, sell the coin
            if daily_profit < -0.02:
                print(Back.BLACK + Fore.RED + f'DAILY PROFIT: {row["ticker"]}' + Fore.RESET)
                crypto_data.loc[index, 'signal'] = -1
